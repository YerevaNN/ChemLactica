...
class TestDataloader(unittest.TestCase):
    ...
    def get_train_dataloader(
        self, train_dataset, batch_size, num_workers, pin_memory
    ) -> DataLoader:
        """
        Returns the training [`~torch.utils.data.DataLoader`].

        Will use no sampler if `train_dataset` does not implement `__len__`,
    ...
    def test_dataloader(self):
        ...
        train_config = {"tokenizer_path": default_tokenizer_path, "block_size": 2048}

        with multiprocessing.Manager() as manager:
            shared_jsonl_files = manager.dict()

            training_data_dirs = [
                os.path.join(TD_PATH, "comp_train"),
                os.path.join(TD_PATH, "assay_train"),
            ]
            ...
            """
                ...
                what we do in training. We exclude accelerate.prepare operation
                because that is for distributed training.
            ...
            initial_train_dataloader = self.get_train_dataloader(
                initial_train_dataset["train"], 16, num_workers=2, pin_memory=True
            )

            """
                keep all the lines in the memory to check if the
                read line by dataset matches the actual line in the file
            ...
            resumed_train_dataloader = self.get_train_dataloader(
                resumed_train_dataset["train"], 16, num_workers=2, pin_memory=True
            )
            ...
            for file_name, lines in loaded_files.items():
                ...
                print(
                    f"File: {file_name}: number of read line {number_of_read}, \
                    number of not read {len(lines) - number_of_read}."
                )
